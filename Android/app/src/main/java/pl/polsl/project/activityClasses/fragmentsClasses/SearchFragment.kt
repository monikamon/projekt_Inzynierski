...
import kotlinx.android.synthetic.main.fragment_list_expenses_search.*
import kotlinx.android.synthetic.main.fragment_list_expenses_search.view.*
...
class SearchFragment: Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        rootView.searchText.addTextChangedListener(
                object: TextWatcher {
                    override fun afterTextChanged(p0: Editable?) {
                        searchString = rootView.searchText.text.toString()
                        if(rootView.searchText.text.isEmpty()){
                            searchString = ""
                        }
                        refresh()
                    }
        ...
        listElements.add(getString(R.string.search_option_1))
        listElements.add(getString(R.string.search_option_2))
        listElements.add(getString(R.string.search_option_3))
        listElements.add(getString(R.string.search_option_4))

        val adapter = ArrayAdapter(context, android.R.layout.simple_spinner_item, listElements)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)

        rootView.searchOptions.adapter = adapter
        rootView.searchOptions.setSelection(0)
    ...
    fun refresh(){
        ...
        for(expense in expanses){
            ...
            if( searchString== ""
                    ...
                    || (searchOptions.selectedItemPosition==2 && expense.price.toString().toUpperCase().contains(searchString.toUpperCase()))
                    || (searchOptions.selectedItemPosition==3 && expense.expenseName.toUpperCase().contains(searchString.toUpperCase()))
            )
